...
class ModelFreeOffPolicy_DQN_RNN(nn.Module):
    ...
    def forward(self, actions, rewards, observs, dones, masks, rtgs):
        """
        ...
        q_pred = q_pred * masks
        # q_target = torch.maximum(q_target, rtgs)  # (T, B, 1)
    ...
    def update(self, batch):
        ...
        actions, rewards, dones, rtgs = batch["act"], batch["rew"], batch["term"], batch["rtg"]
        ...
        outputs = self.forward(actions, rewards, observs, dones, masks, rtgs)
